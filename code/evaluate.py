...
def evaluate(pred_path, ground_truth_path):
    with open(f'./data/{ground_truth_path}') as f:
        ...
        with open(f'output/{pred_path}') as f_pred:
...
def get_missing_ingredients(pred_path, ground_truth_path):
    ...
    with open(f'output/missing_ingredients.json', 'w') as f_out:
        json.dump(missing_substitutes, f_out, ensure_ascii=False)
    print("Missing substitutes can now be found at output/missing_ingredients.json")
...
def get_wrong_ingredients(pred_path, ground_truth_path):
    ...
    with open(f'output/wrong_ingredients.json', 'w') as f_out:
        json.dump(wrong_substitutes, f_out, ensure_ascii=False)
    print("Wrong substitutes can now be found at output/wrong_ingredients.json")
...
def get_precision_recall_each_ingredient(pred_path, ground_truth_path):
    ...
    with open(f'output/metrics_by_ingredient.json', 'w') as f_out:
        json.dump(results, f_out, ensure_ascii=False)
    print("Precision and recall for each ingredient can now be found at output/metrics_by_ingredient.json")
